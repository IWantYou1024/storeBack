...
function msgupdate( id , flag ){
	...
	location.href='${ctx}/home/memoAction_toupdate?id=' + id;
}
 
function msgdel( id ){
	if(confirm("是否确定要删除此条信息?")){
		//_Submit("/home/olmsgDeleteAction.do?delId="+id,null,"删除");
		location.href='${ctx}/home/memoAction_delete?id=' + id;
	}
}
...
<li id="new"><a href="${ctx}/home/memoAction_tocreate">新建</a></li>
...
<li id="stat"><a href="#">历史</a></li>
 ...
 <c:set var="y" value="90"></c:set>
 	<c:forEach items="${memoList}" var="o" >

		<div id='${o.id}' style='position:absolute;left:${x}px;top:${y}px;z-index:1001; height:164px;background:none;' onmousedown='getFocus(this)'>
			<table border=0 cellspacing="0" cellpadding="0" width="220">
				<tr>
					<td style='cursor:move;' onmousedown="MDown(this,'${o.id}')" background="${ctx}/images/olmsg/C0FFE51.gif" height="45">
				...
				<tr>
					<td style='cursor:move;white-space:nowrap;' width='100%' onmousedown="MDown('${o.id}')" background="${ctx}/images/olmsg/C0FFE52.gif" >
						...
						<div style="float:right;width:80px;text-align:right;padding-right:7px;">
							<a style='cursor:pointer;' title="编辑" onclick="msgupdate('${o.id}','edit')"><img src="${ctx}/images/olmsg/doc_edit.gif"/></a>
							<a style='cursor:pointer;' title="删除" onclick="msgdel('${o.id}')"><img src="${ctx}/images/olmsg/doc_del.gif"/></a>
						</div>
					</td>
				</tr>
				<tr>
		...
		<c:set var="x" value="${x + 300}"></c:set>
		<c:if test="${x > 1000}">
			<c:set var="y" value="${y + 200}"></c:set>
			<c:set var="x" value="100"></c:set>
		</c:if>
	</c:forEach>
